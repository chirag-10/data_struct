...
void write_product(){
      f.open("Mart.txt",ios::out|ios::app);
      p.create_product();
...
void modify_record(){
      ...
      cin>>n;
      f.open("Mart.txt",ios::in|ios::out);
      while(f.read((char *) &p ,sizeof(product))){
            if(p.retpno()==n){
                  p.show_product();
...
int main(){
      int ch;
      cout<<"\n\n\t\tD-MART OR MALL DATABASE PROJECT\n";
      do{
            cout<<"\n\n\n\tMAIN MENU\n";
            cout<<"\n\n1.FOR ADMIN";
            cout<<"\n\n2.CUSTOMER";
